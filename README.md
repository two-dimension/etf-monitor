...
面向多只 A 股 ETF 的 15 分钟 K 线价格和成交额异动监控台。当前默认监控 `159915.SZ`（创业板ETF易方达）、`510310.SH`（沪深300ETF易方达）、`588080.SH`（科创50ETF易方达）、`588000.SH`（科创50ETF华夏）、`510300.SH`（沪深300ETF华泰柏瑞）和 `513310.SH`（中韩半导体ETF华泰柏瑞）。后端使用 AkShare 拉取 ETF 分钟行情，成功拉取后会把 K 线保存到本地 SQLite；当 AkShare 不稳定时，接口会回退展示本地缓存，前端状态会显示“本地缓存”。
...
- 当前默认标的：`159915.SZ:创业板ETF易方达,510310.SH:沪深300ETF易方达,588080.SH:科创50ETF易方达,588000.SH:科创50ETF华夏,510300.SH:沪深300ETF华泰柏瑞,513310.SH:中韩半导体ETF华泰柏瑞`。
...
- 放大：9:45 的上午第一根 15 分钟 K 线使用 `OPENING_VOLUME_RATIO_THRESHOLD`，默认 `1.15`；其余时间段使用 `VOLUME_RATIO_THRESHOLD` / `LATE_SESSION_VOLUME_RATIO_THRESHOLD`，默认 `1.3`。9:45 和 13:15 直接与上一交易日同一时间点成交额比较；其他时间段先按日内前一根 K 线环比判断，未触发或日内前一根缺失时，再按上一交易日同一时间点同比判断。
- 成交额放大倍数 >= `CRITICAL_RATIO_THRESHOLD` 时标记为 `critical`，否则为 `warning`。
- 缩额不再生成告警，也不会触发邮件推送。
...
后台轮询全部标的时，会把本轮新增的成交额异动合并到同一封邮件，只包含有异动的标的；若本轮全部标的均无异动，则把无异动状态合并到同一封邮件。收盘后会额外发送一封当天总结，列出全日异动；没有异动也会发送“无异动”总结。同一 `symbol + candle_time` 的告警只记录一次，也只发送一次邮件；同一交易日的收盘总结只发送一次。若 SMTP 发送失败，后端只记录日志，不会中断行情拉取、缓存或告警写入。
