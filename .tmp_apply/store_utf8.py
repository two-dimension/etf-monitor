...
def _sync_alert_volumes(connection: sqlite3.Connection, candles: list[Candle]) -> None:
    seen = {(candle.symbol, candle.time.isoformat()) for candle in candles}
    for symbol, candle_time in seen:
        ...
        previous = _previous_candle_for_alert_sync(connection, symbol, candle_time)
...
def _previous_candle_for_alert_sync(
    connection: sqlite3.Connection, symbol: str, candle_time: str
) -> sqlite3.Row | None:
    ...
    if _uses_previous_trading_day_same_slot(parsed_candle_time):
        return connection.execute(
            ...
            LIMIT 1
            """,
            (symbol, candle_time, f"{parsed_candle_time:%H:%M}"),
        ).fetchone()
    return connection.execute(
        """
        SELECT volume FROM candles
